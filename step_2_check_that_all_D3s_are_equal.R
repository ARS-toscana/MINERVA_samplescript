# in this step the four files D3_CDM.csv are proven to be equal and one of them is saved as D3
...
if (length(CDMs)>1) {
  ...
  for (i in 1:length(combs)) {
    for (CDMcheck in list(combs[[i]])) {
      temp1<-fread(paste0(dirtemp,"D3_",CDMcheck[[1]],".csv"),header = T)
      temp2<-fread(paste0(dirtemp,"D3_",CDMcheck[[2]],".csv"),header = T)
  ...
  temp <- fread(paste0(dirtemp,"D3_ConcePTION.csv"))
  fwrite(temp,file = namefileoutput)
}else{
  #export D3 using any of the existing D3s (since they are all equal)
  temp <- fread(paste0(dirtemp,"D3_",CDMs,".csv"))
  fwrite(temp,file = namefileoutput)
}
